...
inline bool ReadConf(std::string network_topo, std::string network_conf) {
    ...
    // A missing/unreadable config must fail loud: otherwise the eof loop below
    // falls through immediately, ReadConf returns true, and the replay proceeds
    // with ALL network/CC parameters at their hard-coded defaults — producing
    // plausible-but-wrong FCTs with no diagnostic. main() already aborts on a
    // false return.
    if (!conf.is_open()) {
        std::cerr << "[DecoupledReplay] FATAL: cannot open config file '"
                  << network_conf << "'" << std::endl;
        return false;
    }
...
inline void SetupNetwork(void (*qp_finish)(FILE *, Ptr<RdmaQueuePair>),
    ...
    // node_num drives every subsequent allocation; a wrong topology path leaves it
    // zero and SetupNetwork builds an empty/broken network with no diagnostic.
    if (!topof.is_open()) {
        std::cerr << "[DecoupledReplay] FATAL: cannot open topology file '"
    ...
    // RDMA setup (always enabled for decoupled replay)
    {
        ...
        // fopen returns NULL if the output dir is missing; the FCT FILE* is then
        // dereferenced by fprintf in qp_finish at flow-completion time → a deferred
        // segfault far from the cause. Fail loud here instead.
        if (!fct_output || !send_output) {
            std::cerr << "[DecoupledReplay] FATAL: cannot open FCT/send output ('"
                      << fct_output_file << "', '" << send_output_file
