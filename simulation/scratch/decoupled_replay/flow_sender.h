...
inline void SendFlow(int src, int dst, uint64_t maxPacketCount,
    ...
    int pg = 3, dport = 100;
    // send_lat is resolved once in main() (flow-file recorded value + AS_SEND_LAT
    // override); using the shared resolved value keeps every flow consistent with
    // the coupled export run instead of silently defaulting to 6us per flow.
    int send_lat = g_replay_send_lat_us;  // microseconds (multiplied by 1000 to ns below)
    send_lat *= 1000;
    flow_input.idx++;
